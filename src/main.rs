...
fn main() {
    ...
    let tables = get_tables_description(&pg_url).map_err(exit_error).unwrap();
    match dialect {
...
fn get_tables_description(url: &String) -> Result<Vec<TableDescription>, String> {
    ...
    let row_iter = client.query("
        ...
        WHERE table_schema NOT IN ('pg_catalog', 'information_schema') AND c.data_type != 'USER-DEFINED'
        ORDER BY c.table_name;", &[]).map_err(|e| e.to_string())?;
